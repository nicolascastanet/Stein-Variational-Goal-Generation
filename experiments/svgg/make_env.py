...
def make_env(args):
  ...
  if gym.envs.registry.env_specs.get(args.env) is not None:
    ...
    env_fn = lambda: PointMaze2D(maze_type=args.maze_type.lower(), max_steps=args.env_max_step)
    eval_env_fn = lambda: PointMaze2D(test=True,maze_type=args.maze_type.lower(), max_steps=args.env_max_step)
    ...
    eval_env_fn = lambda: Env(max_step=args.env_max_step, internal_goal = internal, 
