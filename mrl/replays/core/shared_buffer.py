...
class SharedMemoryTrajectoryBuffer():
  ...
  def __init__(self, limit, item_shape, n_cpu=1):
    ...
    if 'buffer_bg' in BUFF: # is this a successful trajectory?
      ...
      self.success_idxs = [] # id of successfull trajectories
      self.failed_idxs = [] # id of failed trajectories
      ...
      for buffer_name, batched_values in zip(self.BUFF.items, items):
        if buffer_name == 'buffer_reward':
          ...
          if success:
            ...
            self.success_idxs.append(self.current_trajectory)
          else:
            self.BUFF.buffer_success.append_batch(np.zeros((trajectory_len,), dtype=np.float32))
            self.failed_idxs.append(self.current_trajectory)
  ...
  def sample_succ_trajectories(self, n, group_by_buffer=False, from_m_most_recent=None, success=True):
    """
    Samples n failed / successful full trajectories (optionally from 'from_m_most_recent' trajectories)
    """
    if success == True:
      traj_idxs = self.success_idxs
    else:
      traj_idxs = self.failed_idxs
    try:
      if from_m_most_recent is not None and (len(traj_idxs) - n >= 0):
        min_idx = max(0, len(traj_idxs) - from_m_most_recent)
        if n < len(traj_idxs[-min_idx:]):
          idxs = sample(traj_idxs[-min_idx:],n)
        else:
          idxs = choices(traj_idxs[-min_idx:],k=n)
      else:
        idxs = choices(traj_idxs,k=n)
    except TypeError:
