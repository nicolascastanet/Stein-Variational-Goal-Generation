...
class OnlineHERBuffer(mrl.Module):
  ...
  def sample(self, batch_size, to_torch=True, ent_sampling=False):
    if hasattr(self, 'prioritized_replay') and ent_sampling:
    ...
    if self.goal_space:
      ...
      if has_config_her:
        ...
        if self.config.other_args["inverseRL"]:
          # Use Inverse RL for relabeling

          # Sample IRL batch
          states_iRL, actions_iRL, _, next_states_iRL, dones_iRL, previous_ags_iRL, ags_iRL, goals_iRL = self.inverseRL(batch_size, batch_idxs)
  ...
  def inverseRL(self, batch_size, batch_idxs, ent_sampling=True, num_tasks=10):
    """
    Relabel Transitions using Inverse RL : cf paper Rewriting History with Inverse RL
    """

    # Sample candidate tasks for relabeling from Achieved goal buffer
    tasks_idxs = self.prioritized_replay(num_tasks) # MEP sampling to get diverse tasks
    
    # Sample the real batch (i.e., goals = behavioral goals) to get candidates tasks (tasks = ags)
    _,_, _, _, _, _, achieved_goals, _, _ =\
            self.buffer.sample(num_tasks, batch_idxs=tasks_idxs)
          
    tasks = achieved_goals

    # Sample the real batch (i.e., goals = behavioral goals) to get the transitions to relabel with candidate tasks
    states, actions, rewards, next_states, dones, previous_ags, ags, _, _ =\
            self.buffer.sample(batch_size, batch_idxs=batch_idxs)

    Q_all = torch.empty([num_tasks, batch_size], dtype=torch.float32)

    # get R and Z for each phi
    for t in range(num_tasks): 
        task = tasks[t]
        s_batch = np.concatenate((states, np.tile(task,(batch_size,1))), -1)
        a_batch, s_batch = self.torch(actions), self.torch(s_batch)
        with torch.no_grad():
          if self.config.other_args["alg"].lower() in {'sac', 'td3'}:  
            q1, q2 = self.critic(s_batch, a_batch), self.critic2(s_batch, a_batch)
            Q = torch.min(q1, q2)

          elif self.config.other_args["alg"].lower() == 'ddpg':
            Q = self.critic(s_batch,a_batch)

        Q_all[t] = torch.squeeze(Q)
    
    Q_all = (Q_all - Q_all.mean()) / Q_all.std()  # TODO: 
  
    #Reward scaling : usefull when different tasks have different reward norm
    
    #Z = torch.mean(torch.exp(Q_all), dim=-1)
    #logits = torch.pow(Q_all-torch.log(Z),10)
    logits = torch.swapaxes(Q_all, 0, 1)

    prob = F.softmax(logits, dim=1)
    
    m = torch.distributions.Categorical(prob)
    tasks_index = m.sample()

    return states, actions, rewards, next_states, dones, previous_ags, ags, tasks[tasks_index]
